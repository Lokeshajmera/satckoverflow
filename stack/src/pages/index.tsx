...
export default function Home() {
  ...
  if (!question || question.length === 0) {
    return (
      <Mainlayout>
        <div className="text-center text-gray-500 mt-4">{t("noQuestionFound") || "No questions found."}</div>
      </Mainlayout>
    );
  }
    ...
    <Mainlayout>
      <main className="min-w-0 p-4 lg:p-6 ">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
          <h1 className="text-xl lg:text-2xl font-semibold">{hasMounted ? t("topQuestions") : "Top Questions"}</h1>
          <button
            onClick={() => {
              if (!user) {
        ...
        <div className="w-full">
          <div className="flex flex-col sm:flex-row items-start sm:items-center mb-4 text-sm gap-2 sm:gap-4">
            <span className="text-gray-600">{(question?.length || 0)} {hasMounted ? t("questions").toLowerCase() : "questions"}</span>
            <div className="flex flex-wrap gap-1 sm:gap-2">
              <button className="px-2 sm:px-3 py-1 bg-gray-200 text-gray-700 rounded text-xs sm:text-sm">
                {hasMounted ? t("newest") : "Newest"}
              </button>
